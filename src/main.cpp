    ...
    h.onMessage([&map_waypoints_x,
            ...
            if (length && length > 2 && data[0] == '4' && data[1] == '2') {
                ...
                if (s != "") {
                    ...
                    * This can also be used to recorde if the lanes adjoining the current_lane to the vehicle is free or not.
                    * 
                    * 2. After getting an understanding of the surrounding environment. We will now focus on the current lane. We will check if there is any traffic or car ahead
                    * of our ego vehicle.
                    * 2.a. If there is no car, we can then increase the speed of our ego vehicle a little keeping the jerk to minimum. We will repeat it till it reaches the speed limit.
                    * 2.b. If there is a car, we will check if its speed is faster than our ego vehicle and relative distance between then.
                    ...
                    * - If there is a free lane, we will try to create a lane change path for our ego vehicle.
                    ...
                    */
                    ...
                    bool should_hard_break = false;
                    bool is_left_lane_clear = true;
                    bool is_right_lane_clear = true;
                    bool should_follow_current_lane = false;
                    double front_car_speed = 0.0;
                    float min_safe_dist = 25;

                    // Check if the previous path size was 0 or not
                    if (prev_path_size > 0)
                    {
                        car_s = end_path_s;
                    }
                    ...
                    cout<<"should_hard_brake:"<<should_hard_break<<"\tcar_speed:"<<car_speed<<"\tfront_car_speed:"<<front_car_speed<<"\tref_vel:"<<ref_vel<<"\n";

                    if (should_follow_current_lane && car_speed > front_car_speed)
                    {
                        ref_vel -= should_hard_break ? max(car_speed / 8.0, 0.1) : 0.224 * 1.5;   // Reducing speed with much faster rate than accelaration to avoid collision
                    }
                    ...
                    // Since we now have a good assumption of our ego vehicle and cars in its surrounding, we
                    // now need to plan a route for it to follow.
                    // The path will consists of a list of waypoints evenly spaced at 30m
                    vector<double> ptsx;
                    vector<double> ptsy;
                    ...
                    // Appending 3 more points at the end of the previous path
                    ...
                    tk::spline s;   // Declaring a spline curve
                    s.set_points(ptsx, ptsy);   // Fit spline

                    // Using the points from previous path for smooth transition
                    ...
                    // Extending the previous path to upto 50 way points
                    for (int i = 1; i <= 50 - previous_path_x.size(); i++)
                    {
                        double N = target_dist / (0.02 * ref_vel / 2.24); // Intervals
